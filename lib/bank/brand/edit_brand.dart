...
import 'package:kfa_admin/bank/brand/brand_list.dart';
import '../../components/contants.dart';

typedef OnChangeCallback = void Function(dynamic value);

class Edit_brand extends StatefulWidget {
  Edit_brand(
      {super.key,
  ...
  @override
  State<Edit_brand> createState() => _bank_newState();
}

class _bank_newState extends State<Edit_brand> {
  List? bank_list_get;
  ...
  void initState() {
    ...
    _bankcontxt = TextEditingController(
        text: '${widget.list![index_edit]['bank_brand_contact']}');
    bankcontact = _bankcontxt!.text;
    ///////////////////////////
    _bankname = TextEditingController(
        text: '${widget.list![index_edit]['bank_branch_name']}');
    bankname = _bankname!.text;
    ///////////////////////////
    _bankofficer = TextEditingController(
        text: '${widget.list![index_edit]['bank_brand_officer']}');
    bankofficer = _bankofficer!.text;
    /////////////
    _bank_village = TextEditingController(
        text: '${widget.list![index_edit]['bank_branch_village']}');
...
//bankcontact
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          district_list;
          bank_commune_id =
              widget.list![0]['bank_branch_commune_id'].toString();
          district_id = widget.list![index_edit]['bank_branch_district_id'];
  ...
  Future<void> bank_cummone_value(district) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          cummone_list;
          int? aa = widget.list![index_edit]['bank_branch_commune_id'];
  ...
  Future<void> Edit_new() async {
    Map<String, dynamic> payload = await {
      'bank_branch_name': bankname.toString(),
      'bank_branch_details_id': bankacronym.toString(),
      'bank_brand_officer': bankofficer.toString(),
      'bank_brand_contact': bankcontact.toString(),
      'bank_branch_province_id': int.parse(province_id.toString()),
      'bank_branch_district_id': int.parse(id_district.toString()),
      'bank_branch_village': (bank_village != null)
          ? input_bank_village.toString()
          : input_bank_village,
      'bank_branch_commune_id': int.parse(id_cummone.toString()),
      'bank_branch_published': 0,
    };
    final url = await Uri.parse(
        'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/update_brand/${widget.list![index_edit]['bank_branch_id'].toString()}');
  ...
  void delete_bank() async {
    final response = await http.delete(Uri.parse(
        'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/delete_brand/${widget.list![index_edit]['bank_branch_id'].toString()}'));
    if (response.statusCode == 200) {
      setState(() {});
      Navigator.pop(context);
      Navigator.push(context, MaterialPageRoute(
