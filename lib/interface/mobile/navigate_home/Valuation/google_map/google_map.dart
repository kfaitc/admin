...
import 'package:kfa_admin/interface/mobile/navigate_home/Comparable/comparable3/search_screen.dart/list_markert.dart';
import 'package:pdf/pdf.dart';
import 'package:pdf/widgets.dart' as pw;
import 'package:printing/printing.dart';

import '../../../../../model/executive/comparable_list.dart';
import '../../../../../model/executive/map_comparable_ex.dart';
...
class _SearchPlacesScreenState extends State<Map_Search_Comparable_> {
  ...
  void deleteItemToList(int Id, List list0) {
    setState(() {
      print(list0.toString());

      for (int i = 0; i < list0.length; i++) {
        // print(
        //     'sdfsdffffffffffffffffffffff= ${list0[i]['comparable_id'].toString()}');
        if (list0[i]['propertycomparable_com_id'].toString() == '$Id') {
          list0.removeAt(i);
          widget.listback(list0);
        }
      }
    });
  }

  void deleteItemToList_Map(int Id, List list0) {
    setState(() {
      for (int i = 0; i < list0.length; i++) {
        print(list0[i]['comparable_id'].toString());
        // print(
        //     'sdfsdffffffffffffffffffffff= ${list0[i]['comparable_id'].toString()}');
        if (list0[i]['comparable_id'].toString() == '$Id') {
          list0.removeAt(i);
          // widget.listback(list0);
          widget.list_comparable(list0);
        }
      }
    });
  }
  ...
  void addItemToList(id, markerid, status, creat, s, a) {
    //first
    setState(() {
      widget.back_f_value('one');
      widget.delect_first_time('delect_second');
      if (widget.list == []) {
        print('list');
        list.add({
          // "appraiser_id": 0,
          "propertycomparable_executive_id": int.parse(
            widget.id.toString(),
          ),
          "propertycomparable_com_id": int.parse(
            markerid.toString(),
          ),
        ...
        widget.listback(list);
        /////////////////////////////
        list_com.add({
          "comparable_id": int.parse(markerid.toString()),
          ...
          "comparable_sold_price": comparable_sold_price,
          "provinces_name": provinces_name,
          "district_name": district_name,
          "commune_name": commune_name,
          "comparable_survey_date": comparable_survey_date,
        });
        cm.add(
          Map_Comparable(
            int.parse(markerid.toString()),
        ...
        widget.list_comparable(list_com);
      }
      ...
      ///Seconde
        ...
        widget.list_comparable(widget.list_CP);
      }
    });
    //  print(id);
  }
...
// ${(int.parse(widget.lb.toString()) != 0) ? widget.lb : ''}${(int.parse(widget.total_b.toString()) != 0) ? widget.total_b : ''}
                      ...
                      onChanged: (value) {
                        setState(() {
                          if (value) {
                            ...
                            Navigator.pop(context);
                            // deleteItemToList(markerId, widget.list);
                            deleteItemToList(markerId, widget.list);
                            deleteItemToList_Map(markerId, widget.list_CP);
  ...
  void _delete_open(id) async {
    ...
    final response = await http.delete(Uri.parse(
        'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/delete_executive/$id?propertycomparable_executive_id=${widget.id}'));
    if (response.statusCode == 200) {
      setState(() {
  ...
  void _addMarkers(List<MarkerData> markerDataList, int i, List list) async {
    for (var markerData in markerDataList) {
      Marker newMarker = Marker(
        ...
        onTap: () {
          setState(() {
            property_type_name = list[i]['property_type_name'].toString();
            comparable_land_total = list[i]['comparable_land_total'].toString();
            comparable_sold_total = list[i]['comparable_sold_total'].toString();
            comparable_adding_price =
                list[i]['comparable_adding_price'].toString();
            comparableaddprice = list[i]['comparableaddprice'].toString();
            comparableboughtprice = list[i]['comparableboughtprice'].toString();
            comparable_sold_price = list[i]['comparable_sold_price'].toString();
            provinces_name = list[i]['provinces_name'].toString();
            provinces_name = list[i]['provinces_name'].toString();
            district_name = list[i]['district_name'].toString();
            commune_name = list[i]['commune_name'].toString();
            comparable_survey_date =
                list[i]['comparable_survey_date'].toString();
            showConfirmationBottomSheet(
              i,
              list,
              '21',
    ...
    return Scaffold(
      appBar: (hh != 0)
          ? AppBar(
              ...
              // ),
              // title: Text(
              //   '${widget.list_CP.length} || ${widget.list.length} ',
              // ),
