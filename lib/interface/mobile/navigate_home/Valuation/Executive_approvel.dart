...
import '../property/Map/ToFromDate_ForSale.dart';
import 'detail.dart';

typedef OnChangeCallback = void Function(dynamic value);

// comparable_search Api search
class Executive_approvals extends StatefulWidget {
  Executive_approvals({super.key});

  @override
  State<Executive_approvals> createState() => _ComparableListState();
}

class _ComparableListState extends State<Executive_approvals> {
...
// String? executive_id;
  Future<void> Comparable_search() async {
    try {
      final response = await http.get(Uri.parse(
          'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/search_all${(all_search == 'agency') ? '$first_search$executivestatusid$executive_app$start$end' : ''}${(all_search == 'status') ? '$first_search$executive_app$customerregistered$start$end' : ''}${(all_search == 'executiveapp') ? '$first_search$executivestatusid$customerregistered$start$end' : ''}'));
  ...
  List status_approval_list = [];
  String? name_status;
  Future<void> status_approvals() async {
    try {
      final response = await http.get(Uri.parse(
          'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/executive_approvel'));
      if (response.statusCode == 200) {
        var jsonBody = jsonDecode(response.body);
        status_approval_list = jsonBody;

        setState(() {
          status_approval_list;
        });
      } else {
        print('Error Comparbale List');
      }
          ...
          Expanded(
            ...
            child: DropdownButtonFormField<String>(
              ...
              onChanged: (newValue) {
                setState(() {
                  if (all_search == '') {
                    all_search = 'executiveapp';
                    first_search = '?executive_app=$newValue';
                    text_drop = 'no_text';
                  } else if (all_search == 'executiveapp') {
                    first_search = '?executive_app=$newValue';
                    text_drop = 'no_text';
                  } else {
                    executive_app = '&executive_app=$newValue';
                    text_drop = 'no_text';
                  }
                });
              },
              value: name_status,
              items: status_approval_list
  ...
  Widget comparable_list(BuildContext context) {
    return SingleChildScrollView(
      ...
      children: [
        // Text(
        //     '${(all_search == 'agency') ? '$first_search$executivestatusid$start$end' : ''}${(all_search == 'status') ? '$first_search$customerregistered$start$end' : ''}'),
        // Text('${list_building}'),
        // Text('${list_appraiser}'),
        Padding(
        ...
        _wait_search
            ...
            : Container(
                ...
                child: PaginatedDataTable(
                  ...
                  source: new _DataSource(list, list.length, context, _status,
                      list_appraiser, list_building, list_map),
                ),
              )
      ],
    ));
...
List list_map = [];
Future<void> _list_map_table(id) async {
  var rs = await http.get(Uri.parse(
      'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/excutive/list_Table/$id'));
  if (rs.statusCode == 200) {
    var jsonData = jsonDecode(rs.body);
    list_map = jsonData;
    print('list_map = ${list_map.toString()}');
  }
}

List list_building = [];
Future<void> _building(id) async {
  var rs = await http.get(Uri.parse(
      'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/list_building/$id'));
  if (rs.statusCode == 200) {
    var jsonData = jsonDecode(rs.body);
    list_building = jsonData;
    print('list_building = ${list_building.toString()}');
  }
}

List list_appraiser = [];
Future<void> _appraiser(id) async {
  print('************************************ = $id');
  var rs = await http.get(Uri.parse(
      'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/list_Appraiser/$id'));
  if (rs.statusCode == 200) {
    var jsonData = jsonDecode(rs.body);
    list_appraiser = jsonData;

    print('list_appraiser = ${list_appraiser.toString()}');
  }
}

void Update_status(list, index, context, list_appraiser, list_building,
    list_map, int status_id) async {
  var cusomter_id;
  ...
  if (status_id == 0) {
    ...
    print('222222222222');
    print('list of appraiser = ${list_appraiser.toString()}');
  ...
  final url =
      'https://www.oneclickonedollar.com/laravel_kfa_2023/public/api/update_executive/${list[index]['executive_id'].toString()}'; // Replace with your actual API endpoint URL
  ...
  final Map<String, dynamic> jsonData = {
    ...
    "executive_app": (list[index]['executive_app'].toString() == 'null' ||
        ...
        ? null
        : int.parse(list[index]['executive_app'].toString()),
    "executive_approvel":
    ...
    // "building": [],
    "appraiser": list_appraiser,
    // "appraiser": [],

    "conparable_map": list_map,
    // "conparable_map": [],
  };
  //////////////////////////////////////////////////////////////////
  // final Map<String, dynamic> jsonData = {
  ...
  //   "executive_building": list[index]['executive_building'].toString(),
  //   "executive_by": 0,
  //   "executive_com": 0,
  //   "executive_comment": list[index]['executive_comment'].toString(),
  //   "executive_commune_id":
  //       int.parse(list[index]['executive_commune_id'].toString()),
  //   "executive_create_by": 0,
  //   "executive_customer_id":
  //       int.parse(list[index]['executive_customer_id'].toString()),
  //   "executive_district_id":
  //       int.parse(list[index]['executive_district_id'].toString()),
  //   "executive_fair": list[index]['executive_fair'].toString(),
  //   "executive_fire": list[index]['executive_fire'].toString(),
  //   "executive_forced": list[index]['executive_forced'].toString(),
  //   "executive_land_lengh": list[index]['executive_land_lengh'].toString(),
  //   "executive_land_price": list[index]['executive_land_price'].toString(),
  //   "executive_land_price_per":
  //       list[index]['executive_land_price_per'].toString(),
  //   "executive_land_total": list[index]['executive_land_total'].toString(),
  //   "executive_land_width": list[index]['executive_land_width'].toString(),
  //   "executive_lng": list[index]['executive_lng'].toString(),
  //   "executive_lon": list[index]['executive_lon'].toString(),
  //   "executive_market_max": list[index]['executive_market_max'].toString(),
  //   "executive_market_min": list[index]['executive_market_min'].toString(),
  //   "executive_modify_by": 0,
  //   // "executive_modify_date": "2021-1-2",
  //   "executive_modify_date": "2021-1-2",
  //   "executive_name": 0,
  //   "executive_obligation": list[index]['executive_obligation'].toString(),
  //   "executive_property_name":
  //       list[index]['executive_property_name'].toString(),
  //   "executive_property_type_id":
  //       int.parse(list[index]['executive_property_type_id'].toString()),
  ...
  //      1,
  //   "executive_purpose": list[index]['executive_purpose'].toString(),
  //   "executive_remark": list[index]['executive_remark'].toString(),
  //   "executive_remarks": list[index]['executive_remarks'].toString(),
  //   "executive_road_type_id":
  //       int.parse(list[index]['executive_road_type_id'].toString()),
  //   "executive_user": list[index]['executive_user'].toString(),
  //   "executive_valuate_id":
  //       int.parse(list[index]['executive_valuate_id'].toString()),
  //   "executive_valuation_date":
  //       list[index]['executive_valuation_date'].toString(),
  //   "executive_valuation_issue_date":
  //       list[index]['executive_valuation_issue_date'].toString(),
  //   "executive_zone_id": int.parse(list[index]['executive_zone_id'].toString()),
  //   "executiveapprove1status":
  //       int.parse(list[index]['executiveapprove1status'].toString()),
  //   "executivestatusid": int.parse(list[index]['executivestatusid'].toString()),
  //   "remember_token": "2",
  //   "building": [],
                  ...
                  children: [
                    ...
                    InkWell(
                      onTap: () {
                        ...
                        Update_status(list, index, context, list_appraiser,
                            list_building, list_map, status_id);
                      },
                      child: Container(
...
class _DataSource extends DataTableSource {
  ...
  final List list_building;
  final List list_appraiser;
  final List list_map;
  _DataSource(this.data, this.count_row, this.context, this._status,
      this.list_appraiser, this.list_building, this.list_map);
        ...
        cells: [
          ...
          DataCell(
            ...
            onTap: () async {
              ...
              // _appraiser(item['executive_id'].toString());
              // _building(item['executive_id'].toString());
              // _list_map_table(item['executive_id'].toString());
